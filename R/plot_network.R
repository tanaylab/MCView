plot_propagation_net_metacell <- function(dataset,
    ...
    nn <- nn %>%
        left_join(mc_annot %>% select(mc1 = metacell, col1 = mc_col, cell_type1 = cell_type), by = c("mc1")) %>%
        left_join(mc_annot %>% select(mc2 = metacell, col2 = mc_col, cell_type2 = cell_type), by = c("mc2"))
