...
plot_sample_scatter <- function(dataset,
    ...
    if (any(c(x_type, y_type, color_type) == "Gene")){
        req(cell_types)
        samp_mc_frac <- get_samp_mc_frac(dataset)        
        samp_mc_frac <- samp_mc_frac[, intersect(colnames(samp_mc_frac), selected_mc)]
    }
    ...
    if (x_type == "Metadata") {    
        ...
        egc_x <- get_samples_gene_egc(x_var, dataset, samp_mc_frac) + egc_epsilon        
        df <- df %>%
    ...
    if (y_type == "Metadata") {   
        ...
        egc_y <- get_samples_gene_egc(y_var, dataset, samp_mc_frac) + egc_epsilon
        df <- df %>%
    ...
    if (!is.null(color_var) && color_var != "None") {         
        if (color_type == "Metadata") {
            ...
            egc_color <- get_samples_gene_egc(color_var, dataset, samp_mc_frac) + egc_epsilon
            df <- df %>%
