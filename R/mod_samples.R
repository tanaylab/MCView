...
#'
#' @noRd
mod_samples_server <- function(id, dataset, metacell_types, cell_type_colors, gene_modules, state) {
    moduleServer(
        id,
        function(input, output, session) {
            ...
            setup_samples_composition(input, output, session, ns, dataset, metacell_types, cell_type_colors, group_field, state)
            ...
            # Projection plots
            output$plot_gene_proj_2d <- render_2d_plotly(input, output, session, dataset, metacell_types, cell_type_colors, gene_modules, state, source = "proj_mc_plot_gene_tab", tab_guard = "samples") %>%
            ...
            setup_samples_de(input, output, session, ns, dataset, metacell_types, cell_type_colors, gene_modules, group_field, state)

            # Sample QC metrics panel
            setup_samples_qc(input, output, session, ns, dataset, group_field, state)
