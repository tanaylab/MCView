...
precompute_daf_default_markers <- function(complete_daf, derived_daf,
    ...
    has_scalar <- dafr::has_scalar(complete_daf, "mcview_default_markers")
    has_dist <- dafr::has_matrix(complete_daf, "metacell", "metacell",
                                 "mcview_default_markers_dist")
    if (!force && has_scalar && has_dist) {
        return(invisible(FALSE))
    }
    ...
    # When the scalar already exists, reuse its markers so we don't rewrite
    # it with a (potentially different) re-pick just because the dist matrix
    # was missing. The two artifacts must reference the same marker set, so
    # we only re-pick when the scalar is missing or force=TRUE.
    repicked <- FALSE
    if (!force && has_scalar) {
        markers <- convert_daf_default_markers(complete_daf)
    } else {
    ...
    if (is.null(markers) || length(markers) < 2L) {
        return(invisible(FALSE))
    }
    # New marker list invalidates any pre-existing dist (it was computed against
    # the previous selection). Treat as missing so the dist branch recomputes.
    if (repicked) has_dist <- FALSE

    any_done <- FALSE

    # Write scalar first (cheap) so a subsequent dist-failure still persists
    ...
    if (force || !has_dist) {
        ...
        if (is.null(cor_mat)) return(invisible(any_done))
        dist_obj <- tryCatch(tgstat::tgs_dist(cor_mat), error = function(e) NULL)
        if (is.null(dist_obj)) return(invisible(any_done))
        ...
        ok <- tryCatch({
            ...
            cli::cli_alert_warning("Failed to write default_markers_dist: {e$message}")
            FALSE
        })
        any_done <- any_done || ok
    }
