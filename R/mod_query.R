            ...
            generic_column(
                ...
                generic_box(
                    ...
                    sidebar = shinydashboardPlus::boxSidebar(
                        ...
                        checkboxInput(ns("show_only_fitted"), "Show only fitted genes", value = FALSE)
                    ),
...
#'
#' @noRd
mod_query_server <- function(id, dataset, metacell_types, cell_type_colors, gene_modules, state) {
    moduleServer(
        id,
        function(input, output, session) {
            ...
            mod_query_selection_observers(input, session, state, dataset)
            ...
            group_selectors_mod_query(input, output, session, dataset, ns, group, metacell_types, cell_type_colors, state)
            ...
            # Projection plots
            output$plot_gene_proj_2d <- render_2d_plotly(input, output, session, dataset, projected_metacell_types, atlas_colors, gene_modules, state, group = group, source = "proj_mc_plot_proj_tab", tab_guard = "query")

            # Differential expression
            output$plot_mc_mc_gene_scatter <- render_mc_mc_gene_plotly(input, output, session, ns, dataset, state, gene_modules, mc_mc_gene_scatter_df, metacell_names, atlas_colors, metacell_types = metacell_types, tab_guard = "query")
...
mod_query_selection_observers <- function(input, session, state, dataset, notification_suffix = " in \"Query\" tab") {
...
group_selectors_mod_query <- function(input, output, session, dataset, ns, group, metacell_types, cell_type_colors, state) {
    ...
    setup_group_observers(input, group, "add_metacell_to_group", "remove_group_metacells",
                          "paste_group_metacells", "reset_group", "group_table", "metacell", state)
