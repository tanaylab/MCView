...
#' Allows users to enter gene lists and compute pairwise or module-level
#' correlations across metacells, with interactive heatmap and barplot
#' visualisations and downloadable results.
#'
...
#'
#' @param id Internal parameter for {shiny}.
#' @param dataset Reactive returning the current dataset name.
    ...
    moduleServer(
        ...
        function(input, output, session) {
            ...
            output$correlation_heatmap <- renderPlot({
                ...
                input$heatmap_top_genes, input$cluster_heatmap,
                input$mask_low_correlations, input$correlation_mode
            )
            ...
            # Reactive for filtered gene list (shared by download and table)
            ...
            output$download_gene_list <- downloadHandler(
                filename = function() {
                    paste0("correlated_genes_", Sys.Date(), ".txt")
                },
            ...
            output$download_correlations <- downloadHandler(
                ...
                content = function(file) {
                    ...
                    # Append data
                    write.table(data, file, sep = ",", row.names = FALSE, col.names = TRUE, append = TRUE)
                }
            )
        }
    )
