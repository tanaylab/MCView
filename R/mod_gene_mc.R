...
mod_gene_mc_server <- function(input, output, session, dataset, mc_annot, cell_type_annot) {
    ...
    output$plot_gene_gene_mc <- plotly::renderPlotly({
        ...
        p_gg <- plotly::ggplotly(plot_gg_over_mc(dataset(), values$gene1, values$gene2, mc_annot = mc_annot(), cell_type_annot = cell_type_annot(), plot_text = FALSE), tooltip = "tooltip_text", source = "gene_gene_plot") %>%
            sanitize_for_WebGL() %>%
            plotly::toWebGL() %>%
    ...
    output$plot_gene_age_mc1 <- plotly::renderPlotly({
        ...
        plotly::ggplotly(plot_gene_time_over_mc(dataset(), values$gene1, mc_annot = mc_annot(), cell_type_annot = cell_type_annot()), source = "gene_time_mc_plot1", tooltip = "tooltip_text") %>%
            plotly::hide_legend() %>%
            sanitize_plotly_buttons()
    })
    ...
    output$plot_gene_age_mc2 <- plotly::renderPlotly({
        ...
        plotly::ggplotly(plot_gene_time_over_mc(dataset(), values$gene2, mc_annot = mc_annot(), cell_type_annot = cell_type_annot()), source = "gene_time_mc_plot2", tooltip = "tooltip_text") %>%
            plotly::hide_legend() %>%
            sanitize_plotly_buttons()
    })
    ...
    output$gene_vein_plot <- renderPlot({
        ...
        plot_vein(dataset(), gene = gene, foc_type = foc_type, color_order = color_order, mc_annot = mc_annot())
    })
