...
mod_gene_mc_ui <- function(id) {
    ...
    tagList(
        fluidRow(
            ...
            generic_column(
                width = 5,
                projection_box(ns, "gene_projection", title = "Gene projections", collapsed_accordion = FALSE, show_legend = FALSE)
            )
        )
    )
}
