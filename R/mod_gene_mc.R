...
#'
#' @noRd
mod_gene_mc_server <- function(id, dataset, metacell_types, cell_type_colors, gene_modules, state) {
    moduleServer(
        id,
        function(input, output, session) {
            ...
            mod_gene_mc_selection_observers(input, session, state, dataset)
            ...
            clipboard_changed <- clipboard_changed_2d_reactive(input, state)

            # Projection plots
            output$plot_gene_proj_2d <- render_2d_plotly(input, output, session, dataset, metacell_types, cell_type_colors, gene_modules, state, selected_cell_types = selected_cell_types, source = "proj_mc_plot_gene_tab", tab_guard = "gene_mc") %>%
                bindCache(
                    dataset(),
                    input$color_proj,
                    metacell_types(),
            ...
            scatter_box_outputs(input, output, session, dataset, metacell_types, cell_type_colors, gene_modules, state, ns, plotly_source = "md_md_plot", selected_cell_types = selected_cell_types, tab_guard = "gene_mc")

            atlas_gene_gene(input, output, session, dataset, metacell_types, cell_type_colors, state, ns)
        }
    )
}
...
mod_gene_mc_selection_observers <- function(input, session, state, dataset, notification_suffix = " in \"Genes\" tab") {
...
atlas_gene_gene <- function(input, output, session, dataset, metacell_types, cell_type_colors, state, ns) {
    ...
    scatter_selectors(ns, dataset, output, state, prefix = "atlas_gene_gene")
