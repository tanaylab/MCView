    ...
    tagList(
        generic_column(
            ...
            generic_box(
                ...
                width = 12,
                plotOutput(ns("st_flow_spat"), height = "400px"),
                status = "primary",
                solidHeader = TRUE
            )
        )
    ...
    moduleServer(
        ...
        function(input, output, session) {
            ...
            output$st_flow_spat <- renderPlot({      
                beatle_flow_plot(input, data, dataset, metacell_names, metacell_types, cell_type_colors)
            })
...
summarise_flow_spatial = function(data, to_plot, tb, sb, metacell_types, metacell_names, from = TRUE){
    ...
    }else if(to_plot %in% input$display_select_type_smc %in% metacell_names()){
    ...
    plot_focus = flow[flow$ent2 == to_plot & flow$sbin2 %in% sb & flow$flow > eps & flow$time_bin == tb, ]
    plot_focus = plot_focus %>% group_by(ent2, sbin2) %>% summarise(flow = sum(flow))

    if(from){

        plot_dynamics = flow[flow$ent1 == to_plot & flow$sbin1 %in% sb & flow$flow > eps & flow$time_bin == as.character(as.numeric(tb)+1), ]
        plot_dynamics = plot_dynamics %>% group_by(ent1, ent2, sbin1, sbin2) %>% summarise(flow = sum(flow))
        
    }else{

        plot_dynamics = flow[flow$ent2 == to_plot & flow$sbin2 %in% sb & flow$flow > eps & flow$time_bin == tb, ]
        plot_dynamics = plot_dynamics %>% group_by(ent1, ent2, sbin1, sbin2) %>% summarise(flow = sum(flow))
...
beatle_flow_plot = function(input, data, dataset, metacell_names, metacell_types, cell_type_colors){
    ...
    if(input$mode == 'Types'){
        ...
        color = metacell_types_tab[metacell_types_tab$metacell == to_plot,]$mc_color
    }
    ...
    plot_focus = flow_plot$plot_focus
    ...
    bp = bp + gg_theme()
    # bp = bp + ggtitle(tb) + gg_theme()
    p[[1]] = bp

    g = arrangeGrob(grobs = lapply(p, ggplotGrob), nrow = length(1),
...
scatter_beatle_plot = function(input, data, tbin_time, tb, to_plot_scatter, color, N = 10000){
    ...
    to_plot_scatter = to_plot_scatter[,c('sbin2', 'flow')]
    ...
    p = proj$heatmap(fill = "white") + geom_point(data = points_df, aes(x = x, y = y), colour = color)
