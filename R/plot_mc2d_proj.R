...
mc2d_plot_gene_ggp <- function(dataset, gene, point_size = initial_proj_point_size(dataset), min_d = min_edge_length(dataset), stroke = initial_proj_stroke(dataset), graph_color = "black", graph_width = 0.1, max_lfp = NULL, min_lfp = NULL, max_expr = NULL, min_expr = NULL, scale_edges = FALSE, stat = "expression") {
    ...
    if (stat == "enrichment") {
        ...
        shades <- rev(colorRampPalette(colspec)(100 * abs(max_expr - min_expr) + 1))
        p <- mc2d_df %>%
            mutate(
...
render_2d_plotly <- function(input, output, session, dataset, values, metacell_types, cell_type_colors, source, buttons = c("select2d", "lasso2d", "hoverClosestCartesian", "hoverCompareCartesian", "toggleSpikelines"), dragmode = NULL) {
    plotly::renderPlotly({
        ...
        plot_2d_gene <- function(gene) {
            ...
            fig <- plotly::ggplotly(mc2d_plot_gene_ggp(dataset(), gene, min_lfp = input$lfp[1], max_lfp = input$lfp[2], min_expr = min_expr, max_expr = max_expr, point_size = input$point_size, min_d = input$min_edge_size, stat = input$proj_stat), tooltip = "tooltip_text", source = source)
...
initial_proj_point_size <- function(dataset) {
    if (!is.null(config[[dataset]]$projection_point_size)){
        return(config[[dataset]]$projection_point_size)
    }
...
initial_proj_stroke <- function(dataset){
    if (!is.null(config[[dataset]]$projection_stroke)) {
        return(config[[dataset]]$projection_stroke)
    }
    n_metacells <- length(get_mc_data(dataset, "mc_sum"))
