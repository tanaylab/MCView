...
# ==============================================================================
generate_app_r <- function(datasets, atlases) {
    lines <- c(
    ...
    if (length(datasets) == 1) {
        ...
        lines <- c(lines,
            paste0('daf_path <- file.path(getwd(), "', ds, '")'),
            paste0("daf <- ", open_fn, "(daf_path)")
        )

        if (has_atlas) {
            atlas_name <- paste0(ds, "_atlas")
            lines <- c(lines,
                "",
                paste0('atlas_path <- file.path(getwd(), "', atlas_name, '")'),
                paste0("atlas <- if (dir.exists(atlas_path)) ", open_fn, "(atlas_path) else NULL")
            )
        }
        ...
        for (ds in datasets) {
            lines <- c(lines,
                paste0('daf_list[["', ds, '"]] <- ', open_fn, '(file.path(getwd(), "', ds, '"))')
            )
        }
...
# ==============================================================================
embed_config_in_daf <- function(output_dir, datasets, old_config, verbose = FALSE) {
    for (ds in datasets) {
        daf_path <- file.path(output_dir, ds)
        ...
        daf <- dafr::files_daf(daf_path, mode = "w+")
...
main <- function() {
    ...
    embed_config_in_daf(daf_output_dir, datasets, old_config, verbose = opts$verbose)
    ...
    for (ds in datasets) {
        daf_path <- file.path(daf_output_dir, ds)
        if (dir.exists(daf_path)) {
            tryCatch({
                daf_rw <- dafr::files_daf(daf_path, mode = "w+")
    ...
    app_r_content <- generate_app_r(datasets, atlases)
