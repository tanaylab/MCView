...
__MCView__ supports also importing datasets from the old R `metacell` package, see `MCView::import_dataset_metacell1` for details.
## Run the app
...
```{r, eval=FALSE}
MCView::update_metacell_types("PBMC", "PBMC163k", "/path/to/metacell_types_file")
```
...
```{r, eval=FALSE}
MCView::update_cell_type_colors("PBMC", "PBMC163k", "/path/to/cell_type_colors_file")
```
...
You can then upload the bundle to [shinyapps.io](https://docs.rstudio.com/shinyapps.io/getting-started.html#deploying-applications), shiny-server or any other hosting service. 
